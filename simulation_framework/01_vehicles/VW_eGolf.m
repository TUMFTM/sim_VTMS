...
Parameter.name  =  'VW_eGolf';                                         % Vehicle name
...
Parameter.r_dynamisch = 0.3160;                                     % dynamic wheel radius in m
Parameter.rho_Luft = 1.2;                                           % air density in kg/m^3 (-> value used: standard conditions for 20°C)
Parameter.A_Stirn = 2.2;                                            % vehicle front surface in m^2
Parameter.c_W = 0.27;                                               % drag coefficient
Parameter.m_Fahrzeug = 1615;                                        % vehicle mass in kg
...
Parameter.v_max = 150;                                              % max. speed in km/h
...
Parameter.i_Getriebe = 9.02;                                        % total transmission ratio between electric machine and wheels


% Electric Machine

load('Efficiency_map_EM_VW_eGolf.mat');                             % efficiency map of electric machine
...
Parameter.m_EMaschine = 64;                                         % mass of electric machine in kg
Parameter.m_Getriebe = 32;                                          % mass of transmission in kg


% Power electronics

Parameter.n_MOSFET = 167;                                           % number of MOSFETS in power electronics. This is a model assumption to get the heat dissipation right and comparable between the different vehicles.
...
Parameter.p_Zelle = 38;                                             % Number of cells connected in parallel inside the battery system
Parameter.s_Zelle = 88;                                             % Number of cells connected in serial inside the batter system
...
Parameter.UA_EMaschine_Umgebung_Table = ([0.03;228.19]*1.5);        % Heat transfer capacity of the electric machine to the ambient temperature subject to the vehicle speed in W/K
Parameter.UA_Getriebe_Umgebung_Table = ([1.6;305.7]*1.5);           % Heat transfer capacity of the transmission to the ambient temperature subject to the vehicle speed in W/K
Parameter.UA_EMaschine_Fluid_Table = ([7.5;15]*1.55);               % Heat transfer capacity of the electric machine to the coolant subject to the volume flow inside the coolant cycles in W/K


% UA power electronics

Parameter.UA_c_Umgebung_Table = ([11.5;12.8]*1.5);                  % Heat transfer capacity of the power electronics to the ambient temperature subject to the vehicle speed in W/K in W/K
Parameter.UA_c_Fluid_Table = ([4.25;8.5]*1.55);                     % Heat transfer capacity of the power electronics to the coolant subject to the volume flow inside the coolant cycles in W/K
