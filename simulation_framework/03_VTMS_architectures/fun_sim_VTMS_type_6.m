function [OutputData] = fun_sim_VTMS_type_6(vehicle, load_cycle, charging_info, P_charge_in, Fluid, T_ambient_in, T_init_in, VTMS_name)
...
n_Kuehlkreislauf=2;                                                         % Anzahl an Kühlkreisläufen
...
Fluid_Waermetauscher(1,1).l_Kuehlfluessigkeit=Fluid.Waermetauscher_1.l_Kuehlfluessigkeit;                          % Länge des Kühlflüssigkeitsstroms im 1. Strömungsgebiet des Wärmetauschers in m -> muss ein Vielfaches von l_FinitesVolumen sein
Fluid_Waermetauscher(1,1).A_Kuehlfluessigkeit=Fluid.Waermetauscher_1.A_Kuehlfluessigkeit;               % Querschnittsfläche der Kühlflüssigkeit im 1. Strömungsgebiet des Wärmetauschers in m^2
...
Fluid_Waermetauscher(2,1).A_Kuehlfluessigkeit=Fluid.Waermetauscher_2.A_Kuehlfluessigkeit; % Querschnittsfläche der Kühlflüssigkeit im 2. Strömungsgebiet des Wärmetauschers in m^2
...
Konfig_Kuehlkreislauf{2}={{'Fluid_Batteriepack(1)','Fluid_Schlauch(6)','Fluid_Schlauch(7)','Fluid_Waermetauscher(2,1)','Fluid_Schlauch(8)','Fluid_Schlauch(9)','Fluid_PCM(1)','Fluid_Schlauch(10)','Fluid_Schlauch(11)','Fluid_Kuehler(2)','Fluid_Schlauch(12)'} % Hier müssen die festgelegten Fluid_Komponenten (aus "Definition der Fluid_Komponenten") genannt werden, die in dem jeweiligen Kühlkreislauf vorhanden sind (Angabe Nr_Kuehlkreislauf in der jeweiligen Fluid_Komponente muss mit dem Index von Konfig_Kuehlkreislauf übereinstimmen!) -> Eintrag i in dieser Zeile = Nummer i in den unteren beiden Zeilen 
                          [1,2,3,4,5,6,7,8,2,9,10,11]                                         % Hier werden gerichtete Verbindungen der Fluid_Komponenten und damit die Konfiguration des Kühlkreislaufs definiert -> Eintrag i in dieser Zeile hat mit Eintrag i in unterer Zeile eine gerichtete Verbindung -> Bei einem Dreiwegeventil bzw. bei einer Vereinigung der zwei nach der Aufzweigung durch ein Dreiwegeventil resultierenden Pfade hat ein Eintrag einer Zeile Verbindungen zu zwei Einträgen der anderen Zeile
...
Ventil(1).Vereinigung_vor='Fluid_Schlauch(9)';                              % Name von Fluid_Komponente innerhalb des entsprechenden Kühlkreislaufs, vor der die Vereinigung der zwei nach der Aufzweigung durch das Dreiwegeventil resultierenden Pfade stattfindet
...
% -> Achtung: Zustand eines Dreiwegeventils kann nicht < 0 und nicht > 1 sein bzw. muss >= 0 und <= 1 sein
Ventil(1).Zustand_Ventil_Break_T_FinitesVolumen_Ventil_nach=[273.15+20;273.15+25]; % Der Zustand des Dreiwegeventils ist abh. von der Temperatur der Kühlflüssigkeit bzgl. des Knotens des n. finiten Volumens der Fluid_Komponente, die bei Ventil_nach festgelegt ist
...
UA_Waermetauscher_Kuehlfluessigkeit_Table{1}=Fluid.Waermetauscher.UA_Waermetauscher_Fluid_Table;                   % Die Wärmeübertragungsfähigkeit des Wärmetauschers bzgl. der gesamten Kühlflüssigkeit im 1. bzw. 2. Strömungsgebiet des Wärmetauschers in Abh. von den Volumenströmen der zwei Kühlflüssigkeiten in W/K
...
PCM(1).dT_Faktor_Phasenwechsel=0.0001;                                         % Anteil der Gesamtenthalpie, die im Phasenwechselbereich für den weiteren Themperaturanstieg zur verfügung steht
