function [OutputData] = fun_sim_VTMS_type_3(vehicle, load_cycle, charging_info, P_charge_in, Fluid, T_ambient_in, T_init_in, VTMS_name)
...
Konfig_Kuehlkreislauf{2}={{'Fluid_Batteriepack(1)','Fluid_Schlauch(5)','Fluid_Schlauch(6)','Fluid_Schlauch(7)','Fluid_Schlauch(8)','Fluid_Kuehler(2)','Fluid_Schlauch(9)'} % Hier müssen die festgelegten Fluid_Komponenten (aus "Definition der Fluid_Komponenten") genannt werden, die in dem jeweiligen Kühlkreislauf vorhanden sind (Angabe Nr_Kuehlkreislauf in der jeweiligen Fluid_Komponente muss mit dem Index von Konfig_Kuehlkreislauf übereinstimmen!) -> Eintrag i in dieser Zeile = Nummer i in den unteren beiden Zeilen 
                          [1,2,3,4,2,5,6,7]                                         % Hier werden gerichtete Verbindungen der Fluid_Komponenten und damit die Konfiguration des Kühlkreislaufs definiert -> Eintrag i in dieser Zeile hat mit Eintrag i in unterer Zeile eine gerichtete Verbindung -> Bei einem Dreiwegeventil bzw. bei einer Vereinigung der zwei nach der Aufzweigung durch ein Dreiwegeventil resultierenden Pfade hat ein Eintrag einer Zeile Verbindungen zu zwei Einträgen der anderen Zeile
                          [2,3,4,1,5,6,7,4]};                       
